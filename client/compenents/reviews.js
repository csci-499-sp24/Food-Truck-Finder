// "use client";
// import { Card, Typography, Button } from "@mui/material";
// import Link from "next/link";

// export default function Reviews(props) {
//     const foodTruck = props.foodTruck;
//     const foodTruckId = foodTruck.foodTruck.id;

//     const reviews = foodTruck.reviews && foodTruck.reviews.length > 0 ? (
//         foodTruck.reviews.map((review, index) => (
//             <div key={index} style={{ backgroundColor: '#02092c', padding: '10px', margin: '10px', borderRadius: '10px', color: "white" }}>
//                 <h4>{review.name}: {review.review} </h4>
//                 <p>Rating: {review.rating}</p>
//             </div>
//         ))
//     ) : ( 
//     <p style = {{ textAlign: "center" }}>No reviews available</p>
//     );

//     return (
//         <>
//             <Card sx={{ backgroundColor: "#105372", padding: 2, height: "500px", overflowY: "auto" }}>
//                 <h5 style = {{color: "white", textAlign: "center"}}>REVIEWS and RATINGS</h5>
//                 {reviews}
//                 <div>
//                     <div style = {{ textAlign: "center" }}>
//                         <Link legacyBehavior hrsef={`/foodtruck/${foodTruckId}/addReview`}>
//                             <Button variant="outlined">
//                                 ADD REVIEW 
//                             </Button>
//                         </Link>
//                     </div>
//                 </div>
//             </Card>
//         </>
//     );
// }












"use client";

import { Card, Typography, Button, List, ListItem, ListItemText, Divider } from "@mui/material";
import Link from "next/link";
import "@/styles/reviews.css"

export default function Reviews(props) {
    const foodTruck = props.foodTruck;
    const foodTruckId = foodTruck.foodTruck.id;

    const reviews = foodTruck.reviews && foodTruck.reviews.length > 0 ? (
        <List>
            {foodTruck.reviews.map((review, index) => (
                <ListItem key={index} sx={{ bgcolor: 'background.paper', borderRadius: 1, marginY: 1 }}>
                    <ListItemText
                        primary={<Typography variant="subtitle1">{review.name}</Typography>}
                        secondary={
                            <>
                                <Typography variant="body2">{review.review}</Typography>
                                <Typography variant="body2" color="textSecondary">Rating: {review.rating}</Typography>
                            </>
                        }
                    />
                </ListItem>
            ))}
            <Divider />
        </List>
    ) : (
        <Typography variant="body1" align="center" color="textSecondary">
            No reviews available
        </Typography>
    );

    return (
        <Card sx={{ backgroundColor: "#105372", padding: 2, height: "500px", overflowY: "auto" }}>
            <Typography variant="h5" align="center" color="white" marginBottom={2}>
                REVIEWS and RATINGS
            </Typography>
            {reviews}
            <div style={{ textAlign: "center", marginTop: '1rem' }}>
                <Link href={`/foodtruck/${foodTruckId}/addReview`} passHref>
                    <Button variant="contained" color="primary">
                        ADD REVIEW
                    </Button>
                </Link>
            </div>
        </Card>
    );
}
